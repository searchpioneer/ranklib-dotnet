...
{
    private static readonly ILogger<Evaluator> Logger = NullLogger<Evaluator>.Instance;
    ...
    {
        string[] rType = ["MART", "RankNet", "RankBoost", "AdaRank", "Coordinate Ascent", "LambdaRank", "LambdaMART", "ListNet", "Random Forests", "Linear Regression"
        ];
        RankerType[] rType2 = [RankerType.MART, RankerType.RANKNET, RankerType.RANKBOOST, RankerType.ADARANK, RankerType.COOR_ASCENT, RankerType.LAMBDARANK, RankerType.LAMBDAMART, RankerType.LISTNET, RankerType.RANDOM_FOREST, RankerType.LINEAR_REGRESSION
        ];
        ...
        List<string> savedModelFiles = new List<string>();
        string kcvModelDir = "";
        string kcvModelFile = "";
        string rankFile = "";
        string prpFile = "";
    ...
    public Evaluator(RankerType rType, Metric.Metric trainMetric, Metric.Metric testMetric)
    {
        this.RankerType = rType;
        this.TrainScorer = MetricScorerFactory.CreateScorer(trainMetric);
        this.TestScorer = MetricScorerFactory.CreateScorer(testMetric);
    ...
    {
        this.RankerType = rType;
        this.TrainScorer = MetricScorerFactory.CreateScorer(trainMetric, trainK);
        this.TestScorer = MetricScorerFactory.CreateScorer(testMetric, testK);
    ...
    {
        this.RankerType = rType;
        this.TrainScorer = MetricScorerFactory.CreateScorer(trainMetric, k);
        this.TestScorer = MetricScorerFactory.CreateScorer(testMetric, k);
    ...
    {
        this.RankerType = rType;
        this.TrainScorer = MetricScorerFactory.CreateScorer(metric, k);

        if (!string.IsNullOrEmpty(QrelFile))
        {
            TrainScorer.LoadExternalRelevanceJudgment(QrelFile);
        }
        this.TestScorer = TrainScorer;
    }
    ...
    public void Evaluate(string trainFile, string validationFile, string testFile, string featureDefFile)
    {
        ...
        List<RankList> validation = !string.IsNullOrEmpty(validationFile) ? ReadInput(validationFile) : null;
        List<RankList> test = !string.IsNullOrEmpty(testFile) ? ReadInput(testFile) : null;

        int[] features = ReadFeature(featureDefFile) ?? FeatureManager.GetFeatureFromSampleVector(train);
    ...
    public void Evaluate(string sampleFile, string featureDefFile, int nFold, float tvs, string modelDir, string modelFile)
    {
        ...
        int[] features = ReadFeature(featureDefFile) ?? FeatureManager.GetFeatureFromSampleVector(samples);
    ...
    public void Test(string modelFile, string testFile, string prpFile)
    {
        ...
        var features = ranker.GetFeatures();
        var test = ReadInput(testFile);
    ...
    public void TestWithScoreFile(string testFile, string scoreFile)
    {
        try
        {
            using (var inReader = FileUtils.SmartReader(scoreFile))
            {
                ...
                {
                    var rl = test[i];
                    var scoreArray = new double[rl.Size()];
        ...
        try
        {
            using (var outWriter = new StreamWriter(new FileStream(outputFile, FileMode.Create), System.Text.Encoding.UTF8))
            {
                foreach (var l in test)
                {
                    ...
                    {
                        outWriter.WriteLine($"{l.GetID()}\t{j}\t{ranker.Eval(l.Get(j))}");
                    }
                }
            }
        }
    ...
    public void Rank(string modelFile, string testFile, string indriRanking)
    {
        ...
        try
        {
            ...
            foreach (var l in test)
            {
                ...
                {
                    var k = idx[j];
                    var str = $"{l.GetID()} Q0 {l.Get(k).GetDescription().Replace("#", "").Trim()} {(j + 1)} {SimpleMath.Round(scores[k], 5)} indri";
        ...
        try
        {
            using (var outWriter = new StreamWriter(new FileStream(indriRanking, FileMode.Create), System.Text.Encoding.UTF8))
            {
                foreach (var l in test)
                {
                    for (int j = 0; j < l.Size(); j++)
                    {
                        var str = $"{l.GetID()} Q0 {l.Get(j).GetDescription().Replace("#", "").Trim()} {(j + 1)} {SimpleMath.Round(1.0 - 0.0001 * j, 5)} indri";
                        outWriter.WriteLine(str);
                    }
                }
            }
    ...
    private int[] PrepareSplit(string sampleFile, string featureDefFile, double percentTrain, bool normalize, List<RankList> trainingData, List<RankList> testData)
    {
        ...
        int[] features = ReadFeature(featureDefFile) ?? FeatureManager.GetFeatureFromSampleVector(data);
