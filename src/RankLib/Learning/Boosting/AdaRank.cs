...
public class AdaRank : Ranker
{
    ...
    private WeakRanker? LearnWeakRanker()
    {
        ...
        foreach (var i in _features)
        {
            ...
            {
                double t = _scorer.Score(wr.Rank(_samples[j])) * _sweight[j];
                s += t;
            }
    ...
    private int Learn(int startIteration, bool withEnqueue)
    {
        ...
        for (; t <= NIteration; t++)
        {
            ...
            {
                double tmp = _scorer.Score(bestWR.Rank(_samples[i]));
            ...
            {
                double tmp = _scorer.Score(Rank(sample));
                total += Math.Exp(-alpha_t * tmp);
                trainedScore += tmp;
            }

            trainedScore /= _samples.Count;
            double delta = trainedScore + Tolerance - _lastTrainedScore;
            ...
            {
                double scoreOnValidation = _scorer.Score(Rank(_validationSamples));
            ...
            for (int i = 0; i < _sweight.Length; i++)
            {
    ...
    public override void Learn()
    {
        ...
        {
            int t = Learn(1, true);
            for (int i = _featureQueue.Count - 1; i >= 0; i--)
            {
    ...
    {
        double score = 0.0;
        for (int j = 0; j < _rankers.Count; j++)
        {
