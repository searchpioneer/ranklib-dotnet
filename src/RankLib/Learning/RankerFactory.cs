...
public class RankerFactory
{
	...
	public Ranker CreateRanker(RankerType type) => rFactory[(int)type - (int)RankerType.MART].CreateNew();
	...
	public Ranker CreateRanker(string className)
	{
		try
		{
			var rankerType = (RankerType)Enum.Parse(typeof(RankerType), className);
